...
-- Oppretter tabeller med entitetsintegritet (primÊrn¯kkel)
...
-- Legger pÂ referanseintegritet (fremmedn¯kler)
...
INSERT INTO bok VALUES(DEFAULT,'TÂpenes',1995,7);
...
INSERT INTO bok VALUES(DEFAULT,'NÂlen',1978,3);
...
-- Oppgave a
SELECT tittel FROM bok WHERE bok_id=2;
...
-- Oppgave d
-- Attributter som er unionskompatible: Bok_id og tittel, er uniobskompatible attributter med forlag_id og forlag_navn
-- UNION krever at de er unionskompatible


-- Viser forlag_id og forlag_navn som er lik i både bok og forlag
SELECT forlag_id, forlag_navn FROM forlag
UNION
...
-- Oppgave b
SELECT forlag_id FROM forlag
LEFT JOIN bok USING (forlag_id)
WHERE bok.forlag_id IS NULL;

-- Oppgave c
SELECT fornavn, etternavn from forfattere WHERE fode_aar=1948;

-- Oppgave d
SELECT forlag_navn, adresse
FROM forlag NATURAL JOIN bok
WHERE tittel="Generation X";

-- Oppgave e
SELECT tittel
FROM bok NATURAL JOIN bok_forfatter NATURAL JOIN forfatter
WHERE forfatter.etternavn="Hamsun";

-- Oppgave f
SELECT tittel, utgitt_aar, forlag_navn, adresse, telefon
FROM bok RIGHT JOIN forlag
